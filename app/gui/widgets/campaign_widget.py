...
class CampaignDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.setWindowTitle(
            _("campaigns.create_campaign") if not self.campaign else _("campaigns.edit_campaign")
        )
        self.setModal(True)
        self.resize(700, 800)
        ...
        self.forward_from_message_id_edit.setPlaceholderText(
            _("campaigns.forward_message_id_placeholder")
        )
        forward_form.addRow(
            _("campaigns.forward_message_id") + ":", self.forward_from_message_id_edit
        )
        ...
        self.manual_recipients_edit.setPlaceholderText(
        ...
        try:
            ...
            if selector.exec_() == QDialog.Accepted and selector.selected_chat:
                ...
                QMessageBox.information(
                    self, _("common.information"), _("campaigns.enter_message_id_manually")
                )
    ...
    def preview_spintax(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, _("templates.spintax_preview"), f"Error processing spintax: {e}"
            )
...
<li>Always respect Telegram's terms of service and rate limits</li>
        ...
        msg_box.setGeometry(
            self.x() + (self.width() - 600) // 2, self.y() + (self.height() - 400) // 2, 600, 400
        )
    ...
    def save_campaign(self):
        ...
        try:
            ...
            if self.campaign:
                ...
                self.campaign.campaign_type = CampaignType(
                    self.campaign_type_combo.currentText().lower()
                )
            else:
                # Create new campaign
                self.campaign = Campaign(
            ...
            if self.campaign.use_ab_testing:
                ...
                if variants_text:
                    variants = [
                        {"text": line.strip()} for line in variants_text.split("\n") if line.strip()
                    ]
            ...
            self.campaign.recipient_source = (
                self.recipient_source_combo.currentText().lower().replace(" ", "_")
            )
            ...
            if self.recipient_source_combo.currentText() == "Manual":
                ...
                if csv_file:
                    try:
                        ...
                        QMessageBox.warning(
                            self, "CSV Import Error", f"Error reading CSV file: {e}"
                        )
                        self.campaign.total_recipients = 0
                else:
                    self.campaign.total_recipients = 0
            ...
            QMessageBox.critical(
                self, _("common.error"), _("campaigns.save_failed").format(error=str(e))
            )
...
class CampaignListWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.campaigns_table.setHorizontalHeaderLabels(
            [
                _("common.name"),
                _("common.status"),
                _("campaigns.recipients"),
                _("common.sent"),
                _("common.failed"),
                _("common.progress"),
                _("campaigns.start_time"),
                _("campaigns.last_activity"),
                _("common.actions"),
            ]
        )
    ...
    def load_campaigns(self):
        ...
        try:
            ...
            try:
                ...
                campaigns = session.exec(select(Campaign).where(Campaign.is_deleted.is_(False))).all()
            finally:
                session.close()
            ...
            for row, campaign in enumerate(campaigns):
                ...
                recipients_item.setFlags(
                    recipients_item.flags() & ~Qt.ItemIsEditable | Qt.ItemIsSelectable
                )
                ...
                progress_item.setFlags(
                    progress_item.flags() & ~Qt.ItemIsEditable | Qt.ItemIsSelectable
                )
                ...
                start_time = (
                    ...
                    else _("campaigns.not_scheduled")
                )
                ...
                last_activity = (
                    campaign.last_activity.strftime("%Y-%m-%d %H:%M")
                    if campaign.last_activity
                    else _("campaigns.never")
                )
                ...
                actions = []
                self.campaign_manager.is_campaign_running(campaign.id)
    ...
    def update_button_states(self):
        ...
        if not selected_rows:
            ...
            self.start_button.setStyleSheet(
                "QPushButton { background-color: #6b7280; color: #9ca3af; }"
            )  # Gray
            self.retry_button.setEnabled(False)
        ...
        try:
            with get_session() as session:
                ...
                self.campaign_manager.is_campaign_running(campaign_id)
                self.campaign_manager.can_retry_campaign(campaign_id)
    ...
    def filter_campaigns(self):
        ...
        for row in range(self.campaigns_table.rowCount()):
            ...
            for col in range(
                self.campaigns_table.columnCount() - 1
    ...
    def show_action_menu(self, row, column, campaign_id):
        ...
        self.campaigns_table.item(row, 0).text()
        ...
        if campaign:
            ...
            if campaign.status == CampaignStatus.COMPLETED:
                ...
                duplicate_action.triggered.connect(
                    lambda: self.duplicate_campaign_by_id(campaign_id)
                )
        ...
        menu.exec_(
            self.campaigns_table.mapToGlobal(
                self.campaigns_table.visualItemRect(
                    self.campaigns_table.item(row, column)
                ).bottomLeft()
            )
        )
    ...
    def start_campaign_by_id(self, campaign_id):
        ...
        try:
            ...
            if self.campaign_manager.is_campaign_running(campaign_id):
                QMessageBox.warning(
                    self, _("campaigns.already_running"), _("campaigns.already_running_message")
                )
                return
            ...
            if success:
                QMessageBox.information(
                    self,
                    _("campaigns.start_campaign"),
                    _("campaigns.campaign_started_successfully"),
                )
                self.load_campaigns()  # Refresh the campaigns list
            else:
                QMessageBox.warning(
                    self, _("campaigns.start_failed"), _("campaigns.campaign_start_failed")
                )
        except Exception as e:
    ...
    def retry_campaign_by_id(self, campaign_id):
        ...
        try:
            ...
            if not self.campaign_manager.can_retry_campaign(campaign_id):
                QMessageBox.information(
                    self, _("campaigns.no_retry_needed"), _("campaigns.no_retry_needed_message")
                )
                return
            ...
            if success:
                QMessageBox.information(
                    self, _("campaigns.retry_campaign"), _("campaigns.campaign_retry_successful")
                )
                self.load_campaigns()  # Refresh the campaigns list
            else:
                QMessageBox.warning(
                    self, _("campaigns.retry_failed"), _("campaigns.campaign_retry_failed")
                )
        except Exception as e:
    ...
    def duplicate_campaign_by_id(self, campaign_id):
        ...
        try:
            with get_session() as session:
                ...
                if not campaign:
                    QMessageBox.warning(
                        self,
                        _("campaigns.campaign_not_found"),
                        _("campaigns.campaign_not_found_message"),
                    )
                    return
                ...
                if campaign.status != CampaignStatus.COMPLETED:
                    QMessageBox.warning(
                        ...
                        _("campaigns.cannot_duplicate_message").format(
            ...
            QMessageBox.warning(
                self,
                _("campaigns.error_checking_status"),
                _("campaigns.error_checking_status_message"),
            )
            return
        ...
        if reply == QMessageBox.Yes:
            ...
            if new_campaign_id:
                ...
                QMessageBox.warning(
                    self,
                    _("campaigns.duplication_failed"),
                    _("campaigns.duplication_failed_message"),
                )
    ...
    def pause_campaign_by_id(self, campaign_id):
        ...
        try:
            ...
            if success:
                QMessageBox.information(
                    self, _("campaigns.pause_campaign"), _("campaigns.campaign_paused_successfully")
                )
                self.load_campaigns()  # Refresh the campaigns list
            else:
                QMessageBox.warning(
                    self, _("campaigns.pause_failed"), _("campaigns.campaign_pause_failed")
                )
        except Exception as e:
    ...
    def resume_campaign_by_id(self, campaign_id):
        ...
        try:
            ...
            if success:
                QMessageBox.information(
                    self,
                    _("campaigns.resume_campaign"),
                    _("campaigns.campaign_resumed_successfully"),
                )
                self.load_campaigns()  # Refresh the campaigns list
            else:
                QMessageBox.warning(
                    self, _("campaigns.resume_failed"), _("campaigns.campaign_resume_failed")
                )
        except Exception as e:
    ...
    def stop_campaign_by_id(self, campaign_id):
        ...
        try:
            ...
            if success:
                QMessageBox.information(
                    self, _("campaigns.stop_campaign"), _("campaigns.campaign_stopped_successfully")
                )
                self.load_campaigns()  # Refresh the campaigns list
            else:
                QMessageBox.warning(
                    self, _("campaigns.stop_failed"), _("campaigns.campaign_stop_failed")
                )
        except Exception as e:
    ...
    def start_campaign(self):
        ...
        if not selected_rows:
            QMessageBox.warning(
                self, _("campaigns.no_selection"), _("campaigns.no_selection_start")
            )
            return
        ...
        if not campaign_id:
            QMessageBox.warning(
                self, _("campaigns.invalid_campaign"), _("campaigns.invalid_campaign_message")
            )
            return
        ...
        try:
            with get_session() as session:
                ...
                if not campaign:
                    QMessageBox.warning(
                        self,
                        _("campaigns.invalid_campaign"),
                        _("campaigns.invalid_campaign_message"),
                    )
                    return
                ...
                if campaign.status == CampaignStatus.DRAFT:
                    if campaign.total_recipients > 0:
                        ...
                        if success:
                            QMessageBox.information(
                                self,
                                _("campaigns.campaign_started"),
                                _("campaigns.campaign_started_message"),
                            )
                            self.refresh_campaigns()
                        else:
                            QMessageBox.warning(
                                self,
                                _("campaigns.start_failed"),
                                _("campaigns.start_failed_message"),
                            )
                    else:
                    ...
                    if success:
                        QMessageBox.information(
                            self,
                            _("campaigns.campaign_resumed"),
                            _("campaigns.campaign_resumed_message"),
                        )
                        self.refresh_campaigns()
                    else:
                        QMessageBox.warning(
                            self, _("campaigns.resume_failed"), _("campaigns.resume_failed_message")
                        )
                else:
                    QMessageBox.warning(
                        self, _("campaigns.invalid_action"), _("campaigns.invalid_action_message")
                    )
    ...
    def retry_campaign(self):
        ...
        if not selected_rows:
            QMessageBox.warning(
                self, _("campaigns.no_selection"), _("campaigns.no_selection_retry")
            )
            return
        ...
        if self.campaign_manager.is_campaign_running(campaign_id):
            QMessageBox.information(
                self, _("campaigns.campaign_running"), _("campaigns.campaign_running_message")
            )
            return
        ...
        if reply == QMessageBox.Yes:
            ...
            if success:
                QMessageBox.information(
                    self,
                    _("campaigns.campaign_retry_started"),
                    _("campaigns.campaign_retry_started_message"),
                )
                self.refresh_campaigns()
            else:
                QMessageBox.warning(
                    self, _("campaigns.retry_failed"), _("campaigns.retry_failed_message")
                )
    ...
    def duplicate_campaign(self):
        ...
        if not selected_rows:
            QMessageBox.warning(
                self, _("campaigns.no_selection"), _("campaigns.no_selection_duplicate")
            )
            return
    ...
    def pause_campaign(self):
        ...
        if not selected_rows:
            QMessageBox.warning(
                self, _("campaigns.no_selection"), _("campaigns.no_selection_pause")
            )
            return
            ...
            with get_session() as session:
                ...
                if campaign.status == CampaignStatus.RUNNING:
                    ...
                    if success:
                        QMessageBox.information(
                            self,
                            _("campaigns.campaign_paused"),
                            _("campaigns.campaign_paused_message"),
                        )
                        self.refresh_campaigns()
                    else:
                        QMessageBox.warning(
                            self, _("campaigns.pause_failed"), _("campaigns.pause_failed_message")
                        )
                elif campaign.status == CampaignStatus.PAUSED:
                    # Resume the campaign
                    success = self.campaign_manager.resume_campaign(campaign_id)
    ...
    def stop_campaign(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            if success:
                QMessageBox.information(
                    self, _("campaigns.campaign_stopped"), _("campaigns.campaign_stopped_message")
                )
                self.refresh_campaigns()
            else:
                QMessageBox.warning(
                    self, _("campaigns.stop_failed"), _("campaigns.stop_failed_message")
                )
    ...
    def on_campaign_error(self, campaign_id: int, error_message: str):
        ...
        QMessageBox.warning(
            self, "Campaign Error", f"Campaign {campaign_id} encountered an error:\n{error_message}"
        )
        self.refresh_campaigns()
