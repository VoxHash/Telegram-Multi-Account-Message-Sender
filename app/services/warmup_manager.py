...
class WarmupManager(QObject):
        ...
        try:
            with get_session() as session:
                ...
                account_status = (
                    ...
                    if "." in str(account.status)
                    else str(account.status)
                )
                if account_status != "ONLINE":
                    self.logger.warning(
                        f"Account {account.name} is not online (status: {account_status}), cannot start warmup"
                    )
                    self.warmup_error.emit(
                        account_id, f"Account is not online (status: {account_status})"
                    )
                    return False
                    ...
                    def run(self):
                        ...
                        try:
                            loop.run_until_complete(
                                self.warmup_manager._warmup_account(self.account_id)
                            )
                        finally:
                            loop.close()
            ...
            with get_session() as session:
                ...
                for i in range(messages_to_send):
                    try:
                        ...
                        # Send message
                        result = await self._send_warmup_message(
                            account, test_recipient, warmup_message
                        )
                        ...
                        if result["success"]:
                            ...
                            self.warmup_progress.emit(
                                account_id, account.warmup_messages_sent, target_messages
                            )

                            self.logger.info(
                                f"Warmup message {account.warmup_messages_sent}/{target_messages} sent for account {account.name}"
                            )
                            ...
                            self.logger.warning(
                                f"Failed to send warmup message for account {account.name}: {result.get('error', 'Unknown error')}"
                            )
                        ...
                        self.logger.error(
                            f"Error sending warmup message {i+1} for account {account.name}: {e}"
                        )
                        self.warmup_error.emit(account_id, str(e))
                        break
    ...
    async def _send_warmup_message(
        self, account: Account, recipient: Dict[str, Any], message: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not client:
                ...
                self.logger.debug(
                    f"Client not found, adding account {account.name} to client manager"
                )
                try:
                    ...
                    if not success:
                        return {
                            "success": False,
                            "error": "Failed to add account to client manager",
                        }
                    ...
                    if not client:
                        return {
                            "success": False,
                            "error": "Failed to initialize Telegram client after adding account",
                        }
                except Exception as e:
                    self.logger.error(f"Error adding account to client manager: {e}")
                    return {
                        "success": False,
                        "error": f"Failed to add account to client manager: {e}",
                    }
        ...
        try:
            with get_session() as session:
                ...
                for account in accounts:
                    if not account.is_warmup_complete() and not self.warmup_in_progress.get(
                        account.id, False
                    ):
