...
ONEDRIVE_TOKEN_FILENAME = "onedrive_token.enc"
...
class OneDriveTokenStore:
    """Persist Microsoft OAuth token JSON encrypted under the application data directory."""
    ...
    def __init__(self, cloud_dir: Optional[Path] = None):
        ...
        self.cloud_dir = Path(cloud_dir)
        self.cloud_dir.mkdir(parents=True, exist_ok=True)
        self.token_path = self.cloud_dir / ONEDRIVE_TOKEN_FILENAME
        self.key_path = self.cloud_dir / KEY_FILENAME
    ...
    def load(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            decrypted = decrypt_data(
                self.token_path.read_text(encoding="utf-8"), self._encryption_secret()
            )
            return json.loads(decrypted)
        except (ValueError, json.JSONDecodeError):
            return None
