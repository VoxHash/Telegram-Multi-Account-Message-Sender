...
class SettingsWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.api_id_edit.setText(
            str(self.settings.telegram_api_id) if self.settings.telegram_api_id else ""
        )
        ...
        rate_form_layout.addRow(
            f"{_('settings.global_max_concurrency')}:", self.global_concurrency_spin
        )
        ...
        self.warmup_info.setStyleSheet(
            "QLabel { background-color: #2d2d2d; color: #ffffff; padding: 10px; border-radius: 5px; border: 1px solid #404040; }"
        )
    ...
    def save_settings(self):
        ...
        try:
            ...
            self.settings.telegram_api_id = (
                int(api_id_text) if api_id_text and api_id_text.isdigit() else None
            )
            ...
            QMessageBox.information(
                self, _("settings.settings_saved"), _("settings.settings_saved")
            )
            ...
            QMessageBox.critical(
                self, _("common.error"), _("settings.settings_save_failed").format(error=str(e))
            )
...
"""
    ...
    def reset_settings(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            QMessageBox.information(
                self, _("settings.settings_reset"), _("settings.settings_reset")
            )
    ...
    def on_startup_changed(self, state: int):
        ...
        try:
            ...
            if success:
                ...
                if enabled:
                    QMessageBox.information(
                        self, _("common.success"), _("settings.startup_enabled")
                    )
                else:
                    QMessageBox.information(
                        self, _("common.success"), _("settings.startup_disabled")
                    )
            else:
                # Show error message
        ...
        try:
            ...
            with get_session() as session:
                ...
                accounts = (
                    ...
                    .filter(
                        Account.warmup_enabled.is_(True),
                ...
                self.warmup_status_label.setText(
                    _("settings.starting_warmup").format(count=total_accounts)
                )
                ...
                for account in accounts:
                    ...
                    if not account.is_warmup_complete():
                        ...
                        if warmup_manager.start_warmup(account.id):
                            started_count += 1
                            self.logger.info(
                                f"Successfully started warmup for account {account.name}"
                            )
                        else:
                            self.logger.warning(
                                f"Failed to start warmup for account {account.name}"
                            )
                    else:
                ...
                if started_count > 0:
                    self.warmup_status_label.setText(
                        _("settings.warmup_started").format(count=started_count)
                    )
                    QMessageBox.information(
                        self,
                        _("settings.warmup_started"),
                        _("settings.warmup_started").format(count=started_count),
                    )
                else:
                    self.warmup_status_label.setText(_("settings.no_warmup_needed"))
                    QMessageBox.information(
                        self, _("settings.no_warmup_needed"), _("settings.no_warmup_needed")
                    )

        except Exception as e:
            self.logger.error(f"Error starting warmup: {e}")
            QMessageBox.critical(
                self, _("common.error"), _("settings.error_starting_warmup").format(error=str(e))
            )
            ...
            with get_session() as session:
                ...
                if account:
                    ...
                    if "warmup started" not in current_text.lower():
                        self.warmup_status_label.setText(
                            f"{_('settings.warmup_running')}: {account.name}"
                        )
        except Exception as e:
        ...
        try:
            ...
            with get_session() as session:
                ...
                if account:
                    ...
                    self.logger.info(
                        f"Warmup progress for {account.name}: {sent}/{target} ({progress_percent:.1f}%)"
                    )
                    self.warmup_status_label.setText(
                        f"{_('settings.warmup_progress')}: {account.name} - {sent}/{target} ({progress_percent:.1f}%)"
                    )
        except Exception as e:
        ...
        try:
            ...
            with get_session() as session:
                ...
                if account:
                    ...
                    self.warmup_status_label.setText(
                        f"{_('settings.warmup_completed')}: {account.name}"
                    )
        except Exception as e:
        ...
        try:
            ...
            with get_session() as session:
                ...
                if account:
                    ...
                    self.warmup_status_label.setText(
                        f"{_('settings.warmup_error')}: {account.name} - {error_message}"
                    )
        except Exception as e:
    ...
    def stop_all_warmup(self):
        ...
        try:
            ...
            QMessageBox.information(
                self, _("settings.warmup_stopped"), _("settings.warmup_stopped")
            )

        except Exception as e:
            self.logger.error(f"Error stopping warmup: {e}")
            QMessageBox.critical(
                self, _("common.error"), _("settings.error_stopping_warmup").format(error=str(e))
            )
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                with get_session() as session:
                    ...
                    self.warmup_status_label.setText(
                        f"{_('settings.warmup_reset')}: {reset_count} accounts"
                    )
                    QMessageBox.information(
                        self,
                        _("settings.warmup_reset"),
                        _("settings.warmup_reset").format(count=reset_count),
                    )

        except Exception as e:
            self.logger.error(f"Error resetting warmup: {e}")
            QMessageBox.critical(
                self, _("common.error"), _("settings.error_resetting_warmup").format(error=str(e))
            )
    ...
    def delete_all_logs(self):
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                QMessageBox.information(self, _("settings.logs_deleted_successfully"), message)

                self.logger.info(
                    f"Deleted {len(deleted_files)} log files and {log_count} database logs, freed {size_str}"
                )

        except Exception as e:
            self.logger.error(f"Error deleting logs: {e}")
            QMessageBox.critical(
                self, _("common.error"), _("settings.error_deleting_logs").format(error=str(e))
            )
    ...
    def update_warmup_status(self):
        ...
        try:
            ...
            with get_session() as session:
                ...
                in_progress_accounts = sum(
                    1
                    for account in accounts
                    if warmup_manager.warmup_in_progress.get(account.id, False)
                )
