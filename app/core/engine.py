...
from ..models import Campaign, Account, Recipient
    ...
    async def send_message(
        ...
        try:
            ...
            result = await self.client_manager.send_message(
                account_id, recipient.get_identifier(), message_text, media_path
            )
...
class CampaignRunner:
    ...
    async def run_campaign(self, campaign: Campaign) -> bool:
        ...
        try:
            self.logger.log_campaign_event(
                "start", campaign.id, f"Starting campaign: {campaign.name}"
            )
    ...
    async def _send_campaign_message(
        self, campaign: Campaign, account: Account, recipient: Recipient
    ):
        ...
        try:
            ...
            result = await self.message_engine.send_message(
                account.id, recipient, message_text, media_path
            )
