...
class SendLogWidget(QWidget):
    ...
    def load_send_logs(self):
        ...
        try:
            ...
            from ...models import SendLog, Campaign
            ...
            try:
                ...
                query = query.where(
                    and_(SendLog.created_at >= from_date, SendLog.created_at <= to_date)
                )
    ...
    def update_campaign_combo(self, session):
        ...
        try:
            ...
            current_items = [
                self.campaign_combo.itemText(i) for i in range(self.campaign_combo.count())
            ]
            expected_items = [_("logs.all")] + [
                campaign.name for campaign in campaigns if campaign.name
            ]
            ...
            if set(current_items) != set(expected_items):
                ...
                self.logger.debug(
                    f"Campaign combo updated with {len(seen_campaigns)} unique campaigns"
                )
            else:
    ...
    def export_logs(self):
        ...
        try:
            ...
            if file_path:
                ...
                # Update status
                self.status_label.setText(
                    f"📊 {_('logs.exported_successfully').format(count=exported_count)}"
                )
            ...
            QMessageBox.critical(
                self, _("logs.export_failed"), _("logs.export_failed").format(error=str(e))
            )
