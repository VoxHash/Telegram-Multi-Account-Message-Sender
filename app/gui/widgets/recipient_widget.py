...
from ...models import Recipient, RecipientSource, RecipientStatus, RecipientType
...
class RecipientDialog(QDialog):
            ...
            if selector.exec_() == QDialog.Accepted and selector.selected_chat:
                ...
                if chat.get("type") in ["group", "supergroup", "channel"]:
                    ...
                    QMessageBox.information(
                        self, _("common.information"), _("recipients.user_selection_not_supported")
                    )
    ...
    def load_recipient_data(self):
        ...
        if self.recipient.recipient_type == RecipientType.USER:
            ...
            self.group_id_edit.setText(
                str(self.recipient.group_id) if self.recipient.group_id else ""
            )
            self.group_title_edit.setText(self.recipient.group_title or "")
            self.thread_id_edit.setText(
                str(self.recipient.thread_id) if self.recipient.thread_id else ""
            )
            ...
            if recipient_type == RecipientType.USER:
                if not any(
                    ...
                    QMessageBox.warning(
                        self,
                        "Validation Error",
                        "At least one identifier (username, user ID, or phone) is required for users",
                    )
                    return
            else:
                if not any(
                    ...
                    QMessageBox.warning(
                        self,
                        "Validation Error",
                        "At least one identifier (group ID, title, or username) is required for groups/channels",
                    )
                    return
            ...
            if self.recipient:
                ...
                if recipient_type == RecipientType.USER:
                    ...
                    self.recipient.user_id = (
                        int(self.user_id_edit.text().strip())
                        if self.user_id_edit.text().strip()
                        else None
                    )
                    ...
                    self.recipient.group_id = (
                        int(self.group_id_edit.text().strip())
                        if self.group_id_edit.text().strip()
                        else None
                    )
                    self.recipient.group_title = self.group_title_edit.text().strip() or None
                    self.recipient.thread_id = (
                        int(self.thread_id_edit.text().strip())
                        if self.thread_id_edit.text().strip()
                        else None
                    )
            else:
        ...
        try:
            ...
            if not has_identifier:
                QMessageBox.warning(
                    self,
                    "Mapping Error",
                    "Please map at least one identifier column (username, user_id, or phone_number)",
                )
                return
            ...
            for _row_index, row in df.iterrows():
                ...
                if any(recipient_data.get(field) for field in required_columns):
                    recipient = Recipient(
                        ...
                        user_id=(
                            int(recipient_data["user_id"])
                            if recipient_data.get("user_id") and recipient_data["user_id"].isdigit()
                            else None
                        ),
                    ...
                    if recipient_data.get("tags"):
                        tags_list = [
                            tag.strip() for tag in recipient_data["tags"].split(",") if tag.strip()
                        ]
            ...
            QMessageBox.information(
                self, "Import Complete", f"Successfully imported {len(recipients)} recipients"
            )
            self.accept()
