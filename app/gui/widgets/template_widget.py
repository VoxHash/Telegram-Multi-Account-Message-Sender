...
class TemplateDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.setWindowTitle(
            _("templates.add_template") if not self.template else _("templates.edit_template")
        )
        self.setModal(True)
        self.resize(600, 500)
        ...
        explanation.setStyleSheet(
            "color: #4CAF50; font-weight: bold; padding: 8px; background-color: #1a1a1a; border: 1px solid #4CAF50; border-radius: 4px;"
        )
    ...
    def toggle_spintax_settings(self, enabled: bool):
        ...
        if enabled:
            ...
            if not self.spintax_example_edit.text().strip():
                self.spintax_example_edit.setText(
                    "Hello {name|friend|buddy}, welcome to {our company|our service}!"
                )
    ...
    def validate_spintax_syntax(self):
        ...
        try:
            ...
            if validation_result["patterns_count"] == 0:
                ...
                has_variables = any(
                    var in message_text
                    for var in ["{name}", "{email}", "{phone}", "{company}", "{date}", "{time}"]
                )
                ...
                if has_variables:
                    ...
                    QMessageBox.information(
                        self,
                        _("templates.spintax_validation"),
    ...
    def preview_spintax(self):
        ...
        if not message_text.strip():
            QMessageBox.warning(
                self, _("templates.spintax_preview"), _("templates.no_message_text")
            )
            return
...
class TemplateListWidget(QWidget):
        ...
        try:
            ...
            try:
                ...
                templates = session.exec(
                    select(MessageTemplate).where(MessageTemplate.is_deleted.is_(False))
                ).all()
            finally:
                session.close()
            ...
            for row, template in enumerate(templates):
                ...
                description_item.setFlags(
                    description_item.flags() & ~Qt.ItemIsEditable | Qt.ItemIsSelectable
                )
                self.templates_table.setItem(row, 1, description_item)

                # Message Preview - Disabled text field
                message_preview = (
                    template.body[:100] + "..." if len(template.body) > 100 else template.body
                )
                message_item = QTableWidgetItem(message_preview)
                message_item.setFlags(
                    message_item.flags() & ~Qt.ItemIsEditable | Qt.ItemIsSelectable
                )
                ...
                spintax_item.setFlags(
                    spintax_item.flags() & ~Qt.ItemIsEditable | Qt.ItemIsSelectable
                )
    ...
    def show_action_menu(self, row, column, template_id):
        ...
        menu.exec_(
            self.templates_table.mapToGlobal(
                self.templates_table.visualItemRect(
                    self.templates_table.item(row, column)
                ).bottomLeft()
            )
        )
            ...
            try:
                for _, row in df.iterrows():
                    ...
                    existing = (
                        ...
                        .filter(MessageTemplate.name == row["name"])
                        .first()
                    )
    ...
    def export_csv(self):
        ...
        try:
            ...
            try:
                templates = (
                    session.query(MessageTemplate)
                    .filter(MessageTemplate.deleted_at.is_(None))
                    .all()
                )
                    ...
                    data.append(
                        {
                            ...
                            "tags": (
                                ", ".join(template.get_tags_list())
                                if template.get_tags_list()
                                else ""
                            ),
                            "created_at": (
    ...
    def filter_templates(self):
        ...
        for row in range(self.templates_table.rowCount()):
            ...
            for col in range(
                self.templates_table.columnCount() - 1
