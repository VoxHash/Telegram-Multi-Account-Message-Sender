                        ...
                        self.finished.emit(
                            {
                                ...
                                "details": (
                                    f"Sent to {self.recipient_identifier} at "
...
class TestingWidget(QWidget):
    ...
    def choose_media_file(self):
        ...
        file_path, _selected_filter = QFileDialog.getOpenFileName(
            self, _("testing.choose_media_file"), "", ";;".join(supported_extensions)
        )
                ...
                for recipient in recipients:
                    if recipient.recipient_type == "USER":
                        ...
                        name = (
                            ...
                            or recipient.group_username
                            or f"Group {recipient.id}"
                        )
                    else:  # CHANNEL
                        icon = "📢"
                        name = (
    ...
    def send_test_message(self):
        ...
        try:
            ...
            if not account_id:
                ...
                QMessageBox.warning(
                    self, _("testing.validation_error"), _("testing.please_select_account")
                )
                return
            ...
            if not recipient_identifier and self.recipient_combo.currentIndex() >= 0:
                ...
                recipient_identifier = self.recipient_combo.itemData(
                    self.recipient_combo.currentIndex()
                )
                self.logger.debug(
                    f"Retrieved recipient identifier from index: {recipient_identifier}"
                )

            if not recipient_identifier:
                QMessageBox.warning(
                    self, _("testing.validation_error"), _("testing.please_select_recipient")
                )
                return

            if not message_text:
                QMessageBox.warning(
                    self, _("testing.validation_error"), _("testing.please_enter_message")
                )
                return
            ...
            self.worker = TestMessageWorker(
                account_id, recipient_identifier, message_text, media_path, use_spintax
            )
    ...
    def on_test_finished(self, result):
        ...
        try:
            ...
            test_entry = {
                ...
                "message": (
                    self.message_edit.toPlainText()[:50] + "..."
                    if len(self.message_edit.toPlainText()) > 50
                    else self.message_edit.toPlainText()
                ),
            ...
            if result["success"]:
                QMessageBox.information(
                    self, "Success", f"Test message sent successfully!\n\n{result['details']}"
                )
            else:
                QMessageBox.warning(
                    self,
                    "Failed",
                    f"Test message failed!\n\n{result['message']}\n{result['details']}",
                )
