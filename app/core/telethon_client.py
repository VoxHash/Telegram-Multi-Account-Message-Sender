...
class TelegramClientWrapper:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if self.proxy:
                self.logger.info(
                    f"Using proxy for account {self.account.id}: {self.proxy['proxy_type']}://{self.proxy['addr']}:{self.proxy['port']}"
                )
    ...
    async def authorize(self, phone_code: str, password: Optional[str] = None) -> bool:
        ...
        try:
            if not self._authorized:
                ...
                self.logger.log_telegram_event(
                    "authorize", self.account.id, "Authorization successful"
                )
                return True
    ...
    async def send_message(
        self, peer: str, text: str, media_path: Optional[str] = None
    ) -> Dict[str, Any]:
        """Send a message."""
        ...
        try:
            ...
            self.logger.log_telegram_event(
                "send_message", self.account.id, f"Message sent to {peer}"
            )
            ...
            return {"success": False, "error": "Rate limited", "retry_after": e.seconds}
        except Exception as e:
...
class TelegramClientManager:
    ...
    async def add_account(self, account: Account) -> bool:
        ...
        try:
            ...
            if proxy:
                self.logger.info(
                    f"Using proxy for account {account.id}: {proxy['proxy_type']}://{proxy['addr']}:{proxy['port']}"
                )
            ...
            if await client.connect():
                ...
                self.logger.log_telegram_event(
                    "add_account", account.id, "Account added successfully"
                )
                return True
            else:
    ...
    async def authorize_account(
        self, account_id: int, phone_code: str, password: Optional[str] = None
    ) -> bool:
    ...
    async def send_message(
        self, account_id: int, peer: str, text: str, media_path: Optional[str] = None
    ) -> Dict[str, Any]:
