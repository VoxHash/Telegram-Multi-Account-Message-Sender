...
from PyQt5.QtWidgets import (
    ...
    QComboBox, QGroupBox, QFormLayout, QMessageBox, QProgressDialog,
    ...
    async def _fetch_chats(self):
        ...
        try:
            ...
            if not await client.is_user_authorized():
                self.error_occurred.emit("Account is not authorized. Please authorize the account first.")
                return
            ...
            async for dialog in client.iter_dialogs():
                ...
                if isinstance(entity, Channel) and entity.megagroup:
                    ...
                    try:
                        ...
                        self.logger.warning(f"Could not fetch threads for {chat_info['title']}: {e}")
                        chat_info["threads"] = []
                else:
...
class TelegramSelectorDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.chats_tree.setHeaderLabels([
            _("common.name"),
            _("common.type"),
            _("recipients.members"),
            _("recipients.username")
        ])
            ...
            with get_session() as session:
                ...
                for account in accounts:
                    self.account_combo.addItem(
                        f"{account.name} ({account.phone_number})",
                        account.id
                    )
                ...
                if accounts:
                    ...
                    QMessageBox.warning(
                        self,
                        _("common.warning"),
                        _("recipients.no_online_accounts")
                    )
        except Exception as e:
            ...
            with get_session() as session:
                ...
                if not settings.telegram_api_id or not settings.telegram_api_hash:
                    QMessageBox.warning(
                        self,
                        _("common.warning"),
                        _("recipients.api_credentials_required")
                    )
                    return
                ...
                self.fetcher.error_occurred.connect(
                    lambda error: (progress.close(), QMessageBox.critical(self, _("common.error"), error))
                )
    ...
    def filter_chats(self, text: str):
        ...
        for i in range(self.chats_tree.topLevelItemCount()):
            ...
            if not text or text_lower in chat.get("title", "").lower() or \
               text_lower in (chat.get("username", "") or "").lower():
            ...
            if chat_data.get("has_threads"):
                ...
                for thread in chat_data.get("threads", []):
                    self.thread_combo.addItem(
                        thread.get("title", f"Thread {thread.get('id')}"),
                        thread.get("id")
                    )
            else:
                self.thread_group.setVisible(False)
    ...
    def accept_selection(self):
        ...
        if not selected_items:
            QMessageBox.warning(
                self,
                _("common.warning"),
                _("recipients.select_chat_first")
            )
            return
