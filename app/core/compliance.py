...
class ComplianceChecker:
    ...
    def check_campaign_compliance(self, campaign: Campaign) -> Dict[str, Any]:
        ...
        for rule in self.rules:
            try:
                ...
                warnings.append(
                    {"rule": rule.name, "message": f"Rule check failed: {e}", "severity": "warning"}
                )
    ...
    def _check_message_content(self, campaign: Campaign) -> Dict[str, Any]:
        ...
        if len(campaign.message_text) > 1000:
            return {"compliant": False, "message": "Message is too long (over 1000 characters)"}
    ...
    def _check_recipient_validation(self, campaign: Campaign) -> Dict[str, Any]:
        ...
        if campaign.total_recipients == 0:
            return {"compliant": False, "message": "No recipients specified for campaign"}
...
class SafetyGuard:
    ...
    def block_campaign(self, campaign_id: int, reason: str):
        ...
        self.logger.log_safety_event(
            "campaign_blocked", f"Campaign {campaign_id} blocked: {reason}"
        )
